...
class Employee(ABC):
    ...
    def payout_a_holiday(self) -> None:
        ...
        if self.vacation_days < FIXED_VACATION_DAYS_PAYOUT:
            raise ValueError(
                "You don't have enough holidays left over for a payout"
            )
...
class HourlyEmployee(Employee):
    ...
    def pay(self) -> None:
        print(
            f"Paying employee {self.name} a hourly rate of ${self.hourly_rate} for {self.hours_worked} hours."
        )
...
class Company:
    ...
    def find_employees(self, role: int) -> List[Employee]:
